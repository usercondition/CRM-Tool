...
const { normalizeClientNames, formatClientName, splitLegacyName } = require("./names");
const {
  defaultNumbering,
  normalizeNumbering,
  peekNextNumbers,
...
function rowToClient(row) {
  ...
  const legacyNames = !row.first_name && !row.last_name && row.name ? splitLegacyName(row.name) : null;
  return {
    id: row.id,
    firstName: row.first_name || legacyNames?.firstName || "",
    lastName: row.last_name || legacyNames?.lastName || "",
    name: row.name || formatClientName({ firstName: row.first_name, lastName: row.last_name }) || "",
    email: row.email || "",
    phone: row.phone || "",
...
function rowToOrder(row) {
  return {
    ...
    fulfillmentType: row.fulfillment_type || "Ship",
...
function normalizeSettings(raw) {
  ...
  return {
    ...
    notifyClientOnStatus: raw.notifyClientOnStatus !== undefined ? Boolean(raw.notifyClientOnStatus) : base.notifyClientOnStatus,
    browserNotifications:
      raw.browserNotifications !== undefined ? Boolean(raw.browserNotifications) : base.browserNotifications,
    numbering: normalizeNumbering(raw.numbering || base.numbering),
  };
}
...
function buildClientRecord(data, id, now, existing = null) {
  ...
  const names = normalizeClientNames(data, existing);
  return {
    id,
    ...names,
...
function settingsMixin(base, loadSettings, saveSettingsFn) {
  return {
    ...
    async bootstrapNumberingIfNeeded() {
      const settings = normalizeSettings(await loadSettings());
      if (settings.numbering.bootstrapped) return settings;
      settings.numbering = bootstrapNumberingFromOrders(await base.listOrders(), settings.numbering);
      await saveSettingsFn(settings);
      return settings;
    },
    async peekOrderNumbers() {
      await this.bootstrapNumberingIfNeeded();
      const settings = normalizeSettings(await loadSettings());
      return peekNextNumbers(settings.numbering);
    },
    async allocateOrderNumbers() {
      await this.bootstrapNumberingIfNeeded();
      const settings = normalizeSettings(await loadSettings());
      const allocated = allocateNextNumbers(settings.numbering);
      settings.numbering = allocated.numbering;
      await saveSettingsFn(settings);
      return { orderId: allocated.orderId, invoiceNumber: allocated.invoiceNumber };
    },
  };
}
  ...
  const core = {
    ...
    async createOrder(data) {
      ...
      const order = {
        ...
        fulfillmentType: data.fulfillmentType || "Ship",
...
async function seedPostgresFromJson(pool) {
  ...
  for (const c of seed.clients || []) {
    await pool.query(
      `INSERT INTO clients (id, name, first_name, last_name, email, phone, address, address_line1, address_line2, city, state, zip, notes, created_at, updated_at)
...
async function createPostgresStore(connectionString) {
  ...
  await pool.query(`
    ...
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS fulfillment_type TEXT DEFAULT 'Ship';
  ...
  const core = {
    ...
    async updateClient(id, data) {
      ...
      const updated = buildClientRecord({ ...existing, ...data }, existing.id, todayIso(), existing);
      await pool.query(
        `UPDATE clients SET name=$2, first_name=$3, last_name=$4, email=$5, phone=$6, address=$7, address_line1=$8, address_line2=$9, city=$10, state=$11, zip=$12, notes=$13, updated_at=$14 WHERE id=$1`,
        [
          updated.id,
          updated.name,
          updated.firstName,
    ...
    async createOrder(data) {
      ...
      await pool.query(
        `INSERT INTO orders (id, order_id, client_id, date_received, items, quantity, total_cost, status, payment_status, due_date, notes, tags, invoice_number, po_number, fulfillment_type, public_token, created_at, updated_at)
    ...
    async updateOrder(id, data) {
      ...
      await pool.query(
        `UPDATE orders SET order_id=$2, client_id=$3, date_received=$4, items=$5, quantity=$6, total_cost=$7, status=$8, payment_status=$9, due_date=$10, notes=$11, tags=$12, invoice_number=$13, po_number=$14, fulfillment_type=$15, updated_at=$16 WHERE id=$1`,
        [
          updated.id,
          updated.orderId,
          updated.clientId,
