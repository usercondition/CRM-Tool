...
function ensureJsonFile() {
  ...
  if (!fs.existsSync(DATA_FILE)) {
    fs.writeFileSync(DATA_FILE, JSON.stringify({ clients: [], orders: [], activity: [] }, null, 2) + "\n", "utf8");
  }
}
...
async function createJsonStore() {
  ...
  return activityMixin(core, persistActivities);
}
...
async function createPostgresStore(connectionString) {
  ...
  return activityMixin(core, (orderId, entries) => insertActivities(pool, orderId, entries));
}
